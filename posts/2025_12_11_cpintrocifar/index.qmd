...
Conformal Prediction is the tool we need to accomplish that ! (i am still a noob :) )
...
In short, conformal prediction is a tool that is **model independent** and **distribution-free**. The only assumption on of this framework is that the data points are **exchangeable**, meaning that the vector of data points and any of its permutation has the same distribution (for any distribution). 
...
class ClassificationModule(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.SGD(self.parameters(), lr = self.lr, weight_decay=1e-5, momentum=0.9)

        scheduler = torch.optim.lr_scheduler.PolynomialLR(optimizer=optimizer, total_iters=self.trainer.estimated_stepping_batches)
...
model= ClassificationModule.load_from_checkpoint(
    "ml-runs/804946329265122337/a65eae64ee9d48519ca0af15ed117465/checkpoints/cifar10-epoch=02.ckpt",
...
model= ClassificationModule.load_from_checkpoint(
    "lightning_logs/version_6/checkpoints/cifar100-epoch=00.ckpt",
)
