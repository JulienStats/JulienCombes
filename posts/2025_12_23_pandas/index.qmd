...
models = [
    DummyRegressor(), LinearRegression(), HistGradientBoostingRegressor()
]
...
preds = {
    model_name: X.skb.apply(pre_processing_pipeline).skb.apply(model, y=y)
    for model_name, model in tqdm(models.items())
}
```

We have now to select which model gives the best generalization results by cross validating it. 

Cross validate methods returns a pandas dataframe with the results for the specified esimator. We can make a big big big dataframe with all the results from cross validatoin of each models : 
...
result_df = [
    ...
    for model_name, pred in tqdm(preds.items()) 
]

result_df = pd.concat(result_df)
TableReport(result_df)
```
